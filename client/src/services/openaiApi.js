...
api.interceptors.response.use(
  (response) => {
    console.log(
      `[OpenAI API Response] ${response.status} ${response.config.url}`
    );
    return response;
  },
  (error) => {
    console.error(
      `[OpenAI API Error] ${error.response?.status} ${error.config?.url}`,
      error.response?.data
    );
    return Promise.reject(error);
  }
...
export const enhancePrompt = async (sceneDescription, character) => {
  try {
    ...
    throw new Error(
      error.response?.data?.error || "프롬프트 강화에 실패했습니다."
    );
  }
};
...
export const generateVisionCaption = async (file, characterName) => {
  try {
    ...
    throw new Error(
      error.response?.data?.error || "이미지 캡션 생성에 실패했습니다."
    );
  }
};

      ...
      immer((set, get) => ({
        ...
        addCharacter: (character) => {
          set((state) => {
            const newCharacter = {
              ...
              description: character.description || `A character named ${character.name}`,
              ...character,
            };
        ...
        updateCharacter: (id, updates) => {
          set((state) => {
            ...
            if (characterIndex !== -1) {
              state.characters[characterIndex] = { ...state.characters[characterIndex], ...updates };
            }
          });
        },
        ...
        generateScene: async (sceneIndex, selectedAsset) => {
          ...
          try {
            ...
            // 프롬프트 강화
            const enhancedPrompt = await openaiApi.enhancePrompt(sceneDescription, mentionedCharacter);
        ...
        enhancePrompt: async (sceneDescription, character) => {
          ...
          try {
            const result = await openaiApi.enhancePrompt(sceneDescription, character);
            set((state) => {

  ...
  devtools(
    persist(
      immer((set, get) => ({
        ...
        get filteredAssets() {
          ...
          return assets.filter(asset => {
            ...
            const matchesSearch = !filters.search || 
              ...
              asset.triggerWord.toLowerCase().includes(filters.search.toLowerCase());
        ...
        toggleFavorite: async (assetId, isProjectAsset = false, projectId = null) => {
          set((state) => {
          ...
          try {
            ...
            set((state) => {
              if (isProjectAsset) {
                const assetIndex = state.projectAssets.findIndex(a => a.id === assetId);
                if (assetIndex !== -1) {
                  state.projectAssets[assetIndex].isFavorite = result.asset.isFavorite;
                }
              } else {
        ...
        selectAsset: (asset) => {
          set((state) => {
            const isSelected = state.selectedAssets.find(a => a.id === asset.id);
            if (isSelected) {
              state.selectedAssets = state.selectedAssets.filter(a => a.id !== asset.id);
            } else {
